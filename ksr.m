...
% 	%given spread of data and number of kernels lets evenly spead with width
% 	%being 3sd?
 	%h=((max(y)-min(y))/r.n)*3;
 	%h=sqrt(hy);
	%let's just span our space evenly
...
for k=1:N
    z=kerf((r.x(k)-x(~xnan))/h);
    r.f(k)=sum(z(~ynan).*y(~ynan))/sum(z);
end
