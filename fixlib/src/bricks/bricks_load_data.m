%   BRICKS_LOAD_DATA loads data from a bricks-droll ".data" file
%   ALL = BRICKS_LOAD_DATA(FILENAME, 'all') loads all the data from
%   FILENAME and places the result in ALL, which is a 1D cell-array
%   with 1 cell per column, in the original order.
%
%   ANS = BRICKS_LOAD_DATA(FILENAME, OPTION) loads one column of
...
% $Id: bricks_load_data.m,v 1.1 2003/01/20 19:41:48 sullivan Exp $
% pskirko 6.4.01

function A = bricks_load_data(filename, option)
...
if(strcmp(option, 'all'))
  ...
  s21 = '%d '; s22 = '%d '; s23 = '%d '; s24 = '%s '; s25 = '%s '; 
  s26 = '%f '; s27 = '%f '; s28 = '%f '; s29 = '%f '; s30 = '%f ';
  s31 = '%f ';
  ...
  [s1 s2 s3 s4 s5 s6 s7 s8 s9 s10 ...
   s11 s12 s13 s14 s15 s16 s17 s18 s19 s20 ...
   s21 s22 s23 s24 s25 s26 s27 s28 s29 s30 s31] = textread(filename, format);
  ...
  elseif(strcmp(option, 'pos_z'))
    s21 = '%d ';
  elseif(strcmp(option, 'height'))
    s22 = '%d ';
   elseif(strcmp(option, 'width'))
    s23 = '%d ';
  elseif(strcmp(option, 'color'))
    s24 = '%s ';
   elseif(strcmp(option, 'texture'))
    s25 = '%s ';  
