...
function [fix, fix_frames, vel] = fix_finder_vt(t, x, raw_asl, ...
    clump_space_thresh, ...
    clump_t_thresh, ...
    tl_pupil_thresh, ...
    vt_t_thresh, ...
...
vel_orig = compute_vel(t, x);
vel = vel_orig.*1000; % deg/ms -> deg/s
%vel_tmp=medfilt1(vel,10,'omitnan');
%vel_nan=isnan(vel);
%vel_tmp(vel_nan)=NaN;
%vel=vel_tmp;
